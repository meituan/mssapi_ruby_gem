...
namespace :docs do
  task :update_readme => :update_region do
    ...
    MSS::SERVICES.values.each do |svc|
      apis = Dir.glob("./lib/mss/api_config/#{svc.class_name}*.yml")
    ...
    supported.sort_by{|svc,api| svc.downcase }.each do |(class_name,apis)|
      ...
      rows << "    <tr>"
      rows << "      <td#{rowspan}>MSS::#{class_name}</td>"
      rows << "      <td>#{apis.first}</td>"
      rows << "      <td#{rowspan}>#{MSS::SERVICES[class_name].full_name}</td>"
      rows << "    </tr>"
  ...
  task :update_region do
    ...
    svcs = []
    MSS::SERVICES.values.sort_by(&:method_name).each do |svc|
