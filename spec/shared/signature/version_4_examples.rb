...
module MSS
  shared_examples_for "an authorize v4 request" do
    ...
    let(:credentials) {
      MSS::Core::CredentialProviders::StaticProvider.new({
        :access_key_id => 'akid',
    ...
    context '#add_authorization!' do
      ...
      it 'popualtes the authorization header' do
        ...
        request.add_authorization!(credentials)
        request.headers['authorization'].should eq("MSS4-HMAC-SHA256 Credential=akid/20120101/region-name/service-name/mss4_request, SignedHeaders=content-type;host;x-amz-content-sha256;x-amz-date, Signature=4cbc193e0b128f76c834341c3d895d67a0f783a558bee9563c829918c5005b9a")
      end

      it 'includes the session token in the signature when present' do
        credentials.stub(:session_token).and_return('SESSION')
        request.add_authorization!(credentials)
        request.headers['authorization'].should eq("MSS4-HMAC-SHA256 Credential=akid/20120101/region-name/service-name/mss4_request, SignedHeaders=content-type;host;x-amz-content-sha256;x-amz-date;x-amz-security-token, Signature=f6e66719afab6f610e45783ee80b4b365997a53b2f7912412989700badfd0284")
      end
