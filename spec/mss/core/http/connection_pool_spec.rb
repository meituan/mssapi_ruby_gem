...
describe MSS::Core::Http::ConnectionPool do

  def pool options = {}
    MSS::Core::Http::ConnectionPool.new(options)
  end
  ...
  context '.for' do
    ...
    it 'returns a connection pool' do
      pool.should be_a(MSS::Core::Http::ConnectionPool)
    end
  ...
  context '.pools' do
    ...
    it 'returns an array of connection pools' do
      pool = pool()
      pools = MSS::Core::Http::ConnectionPool.pools
