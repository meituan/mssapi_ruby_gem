...
module MSS::Core
    ...
    context 'adding statements' do
      ...
      context '#allow' do
        ...
        it 'should let you set multiple conditions' do
          condition_block.should_receive(:add).with(:is, "s3:prefix", "foo")
          condition_block.should_receive(:add).with(:is, "mss:SecureTransport", true)
          policy.allow.where("s3:prefix").is("foo").
            where("mss:SecureTransport").is(true)
        end
  ...
  describe Policy::ConditionBlock do
    ...
    context '#[]' do
      ...
      before(:each) do
        block.add(:is, "mss:Foo", "bar")
        block.add(:lte, "mss:Bar", 12)
        block.add(:gt, "mss:Bar", 2)
        block.add(:is, "s3:something-else", true)
        block.add(:like, "mss:Zap", "a*b")
      end

      it 'should return another condition block' do
        block["mss:Foo"].should be_a(Policy::ConditionBlock)
      end

      it 'should filter out operators where no triples match' do
        block["mss:Bar"].operators.should_not include("StringEquals",
                                                      "Bool",
                                                      "StringLike")
      end

      it 'should filter out keys where no triples match' do
        block[:is].keys.should_not include("mss:Bar",
                                           "mss:Zap")
      end
      ...
      context 'symbol operator filters' do
        ...
        before(:each) do
          block.add(:is_ip_address, "mss:SourceIp", "1.2.3.4/0")
          block.add(:not_ip_address, "mss:SourceIp", "4.3.2.1/32")
          block.add("DateGreaterThan", "mss:CurrentTime", "2010-10-12")
        end
    ...
    context 'with a condition' do
      ...
      context 'multiple keys for the same operator' do
        ...
        it 'should merge them' do
          block.add("StringNotEquals", "s3:prefix", "badstuff", "otherstuff")
          block.add("StringNotEquals", "mss:UserAgent", "Mozilla")
          block.to_h.should == {
            "StringNotEquals" => {
              "s3:prefix" => ["badstuff", "otherstuff"],
              "mss:UserAgent" => ["Mozilla"]
            }
          }
        end
      ...
      context 'symbol operators' do
        ...
        context 'Time' do
          ...
          it 'should be converted to ISO 8601' do
            block.add(:is, "mss:CurrentTime", example_value)
            # it's hard to validate the time zone
            block.to_h["DateEquals"]["mss:CurrentTime"].first.should =~ /^2010-10-12T\d+:\d+:\d+/
          end
        end
        ...
        context 'Date' do
          ...
          it 'should be converted to ISO 8601' do
            block.add(:is, "mss:CurrentTime", example_value)
            block.to_h["DateEquals"]["mss:CurrentTime"].should == ["2010-10-12"]
          end
        end
      ...
      context 'symbol keys' do
        ...
        it 'should use the mss:* prefix by default' do
          block.add(:gt, :current_time, Time.now)
          block.to_h["DateGreaterThan"].keys.should include("mss:CurrentTime")
        end
