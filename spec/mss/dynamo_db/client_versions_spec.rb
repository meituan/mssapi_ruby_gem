  ...
  class DynamoDB
    describe Client do
      ...
      it 'constructs a client using version from config' do
        config = MSS.config.with(:dynamo_db => { :api_version => '2012-08-10' })
        client = MSS::DynamoDB::Client.new(:config => config)
      ...
      context 'with default versions' do
        ...
        def set_default(api_version)
          MSS.config.stub(:dynamo_db).and_return(:api_version => api_version)
        end
        ...
        it 'uses :api_version from new over MSS.config' do
