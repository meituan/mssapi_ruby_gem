...
module MSS
  class SNS
    describe Topic do
      ...
      let(:arn) { 'arn:mss:sns:us-east-1:123456789012:topicname' }
      ...
      context '#name' do
        ...
        it 'returns the trailing piece from the arn' do
          topic = Topic.new('arn:mss:sns:us-east-1:599169622985:MyTopic')
        ...
        context 'endpoint' do
          ...
          context 'sqs' do
            ...
            it 'should use the "sqs" protocol' do
              params_for_endpoint("arn:mss:sqs:foo:bar:baz").should == {
                :endpoint => "arn:mss:sqs:foo:bar:baz",
                :protocol => "sqs",
                :topic_arn => arn
              }
            end
            ...
            it 'should accept an SQS::Queue object' do
              ...
              queue.stub(:arn).and_return("arn:mss:sqs:foo:bar:baz")
            ...
            it 'should raise ArgumentError if the ARN is not SQS-like' do
              lambda { topic.subscribe("arn:mss:sns:foo:bar:baz") }.
                should raise_error(ArgumentError, "expected a queue ARN")
            end

            it 'accepts ARNs' do
              lambda {
                topic.subscribe("arn:mss-cn:sqs:foo:bar:baz")
      ...
      context 'attributes' do
        ...
        let(:policy) { Policy.from_json('{"Version":"2008-10-17","Id":"us-east-1/698519295917/test__default_policy_ID","Statement" : [{"Effect":"Allow","Sid":"us-east-1/698519295917/test__default_statement_ID","Principal" : {"MSS": "*"},"Action":["SNS:GetTopicAttributes","SNS:SetTopicAttributes","SNS:AddPermission","SNS:RemovePermission","SNS:DeleteTopic","SNS:Subscribe","SNS:ListSubscriptionsByTopic","SNS:Publish","SNS:Receive"],"Resource":"arn:mss:sns:us-east-1:698519295917:test","Condition" : {"StringLike" : {"MSS:SourceArn": "arn:mss:*:*:698519295917:*"}}}]}') }
        ...
        before(:each) do
          response.data[:attributes] = {
            ...
            'Owner' => '1234567890',
            'TopicArn' => 'arn:mss:sns:us-east-1:123456789012:topicname',
