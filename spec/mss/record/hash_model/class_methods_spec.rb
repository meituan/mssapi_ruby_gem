...
module MSS
  module Record
    describe Model do
      ...
      before(:each) do
        MSS.config(:access_key_id => "id", :secret_access_key => "secret")
        klass.stub(:name).and_return("ExampleClass")
      end

      after(:each) do
        MSS.config(:access_key_id => nil, :secret_access_key => nil)
      end

      let(:klass) { Class.new(MSS::Record::HashModel) }
      ...
      context 'create_table' do
        ...
        it 'prefixes the shard name with MSS::Record.table_prefix' do
          MSS::Record.stub(:table_prefix).and_return('prefix-')
          tables.should_receive(:create).with(
      ...
      context 'dynamo_db_table' do
        ...
        it 'returns a table object' do
          klass.dynamo_db_table.should be_a(MSS::DynamoDB::Table)
        end
        ...
        it 'applies the table prefix to the shard name' do
          MSS::Record.stub(:table_prefix).and_return('test-')
      ...
      context '[]' do
        ...
        it 'removes the table prefix' do
          MSS::Record.stub(:table_prefix).and_return('prefixed-')
