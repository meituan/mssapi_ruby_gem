  ...
  class S3
    ...
    describe Client do
      ...
      it 'should be accessible from the configuration' do
        config = MSS.config.with(
                                 :access_key_id => 'foo',
                                 :secret_access_key => 'bar')
        config.s3_client.should be_a(S3::Client)
      end

      it_should_behave_like "an mss client", :list_buckets
...
<WebsiteConfiguration xmlns=\"http://s3.amazonmss.com/doc/2006-03-01/\">
  <IndexDocument>
  ...
  <CORSRule>
    ...
    <AllowedOrigin>http://foo.com</AllowedOrigin>
      ...
      context 'logging', :logging => true do
        ...
        let(:xml) { <<-XML.strip }
<BucketLoggingStatus xmlns="http://doc.s3.amazonmss.com/2006-03-01">
  <LoggingEnabled>
  ...
  <TagSet>
    ...
    <Tag>
      ...
      shared_examples_for "an s3 http request" do |verb|
        ...
        it_should_behave_like "an mss http request", verb
      ...
      shared_examples_for "requires bucket_name" do
        ...
        it 'puts dns compat bucket name in the host' do
          ...
          request_host.should == 'dns-compat.s3.amazonmss.com'
        end
      ...
      context '#get_bucket_location' do
        ...
        it 'returns nil for missing location constraint' do
          new_client = client.with_http_handler do |req, resp|
            resp.body = <<-XML.strip
              ...
              <LocationConstraint xmlns="http://s3.amazonmss.com/doc/2006-03-01/"/>
            XML
          end
        ...
        it 'returns strings for buckets with location constraints' do
          new_client = client.with_http_handler do |req, resp|
            resp.body = <<-XML.strip
              <?xml version="1.0" encoding="UTF-8"?>
              <LocationConstraint xmlns="http://s3.amazonmss.com/doc/2006-03-01/">EU</LocationConstraint>
            XML
          end
          r = new_client.get_bucket_location(:bucket_name => 'foo')
      ...
      context '#dns_compatible_bucket_name?' do
        ...
        let (:compatible_names) { ["mymssbucket",
                                   "my.mss.bucket",
                                   "mymssbucket.1",
                                   "foo",
                                   "1234",
        ...
        let (:incompatible_names) { [".mymssbucket",
                                     "mymssbucket.",
                                     "my..mssbucket",
                                     "it",
                                     "r"*64,
      ...
      it 'retries request timeout errors' do
        ...
        resp.error = MSS::S3::Errors::RequestTimeout.new
