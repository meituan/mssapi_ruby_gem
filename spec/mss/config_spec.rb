...
describe MSS do
  context 'configuration' do
    ...
    def config options = {}
      MSS.config.with(options)
    end

    context 'options' do

      MSS::SERVICES.values.map(&:method_name).each do |option|
      ...
      context ':credential_provider' do
        ...
        it 'defaults to an MSS::Core::CredentialProviders::DefaultProvider' do
          config.credential_provider.should
            be_a(MSS::Core::CredentialProviders::DefaultProvider)
        end
      ...
      context ':http_handler' do
        ...
        it 'defaults to a NetHttpHandler' do
          ...
          handler.should be_a(MSS::Core::Http::NetHttpHandler)
      ...
      context ':log_formatter' do
        ...
        it 'defaults to the default log formatter' do
          config.log_formatter.should eq(MSS::Core::LogFormatter.default)
        end
