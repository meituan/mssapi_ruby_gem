      ...
      context "error parsing" do
        ...
        it 'should raise the correct error class' do
          http_handler.stub(:handle) do |req, resp|
            resp.status = 400
            resp.body = <<END
<ErrorResponse xmlns="http://queue.amazonmss.com/doc/2009-02-01/"><Error><Type>Sender</Type><Code>Foo</Code><Message>BAR</Message><Detail/></Error><RequestId>abc123</RequestId></ErrorResponse>
END
          end
        ...
        it 'should strip the MSS.SimpleQueueService prefix when forming the class name' do
          http_handler.stub(:handle) do |req, resp|
            resp.status = 400
            resp.body = <<-XML
<ErrorResponse xmlns="http://queue.amazonmss.com/doc/2009-02-01/">
  <Error>
    <Type>Sender</Type>
      <Code>MSS.SimpleQueueService.Foo</Code>
      <Message>BAR</Message>
      <Detail/>
  </Error>
      ...
      context 'error handling' do
        it 'should refresh credentials when it is expired' do
          ...
          http_handler.should_receive(:handle) do |req, resp|
            resp.body = <<END
<ErrorResponse xmlns="http://queue.amazonmss.com/doc/2009-02-01/"><Error><Type>Sender</Type><Code>ExpiredToken</Code><Message> The security token included in the request is expired</Message><Detail/></Error><RequestId>abc123</RequestId></ErrorResponse>
END
            resp.status = 400
          end.ordered
          http_handler.should_receive(:handle) do |req, resp|
